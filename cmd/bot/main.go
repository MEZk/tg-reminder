...
func execute(ctx context.Context, dbFile, migrationsFolder, tgAPIToken string, debug bool) error {
	...
	if backupDir := os.Getenv(envBackupDir); backupDir != "" {
		backupRetentionInterval, err := time.ParseDuration(os.Getenv(envBackupRetention))
		if err != nil {
			return fmt.Errorf("can't parse backup retention interval: %w", err)
		}

		backupInterval, err := time.ParseDuration(os.Getenv(envBackupInterval))
		if err != nil {
			return fmt.Errorf("can't parse backup interval: %w", err)
		}

		backup, err := backuper.New(db, backupDir, backupInterval, backupRetentionInterval)
		if err != nil {
			return fmt.Errorf("can't create backuper: %w", err)
		}
		// backuper sender starts in background goroutine
		go func() {
			backup.Run(ctx)
		}()
	}
