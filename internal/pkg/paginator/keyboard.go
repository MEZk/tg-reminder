...
func (p *Paginator) buildKeyboard() models.InlineKeyboardMarkup {
	...
	var controlButtonsRow []models.InlineKeyboardButton
	if p.editButtonText != "" {
		controlButtonsRow = append(controlButtonsRow, models.InlineKeyboardButton{
			Text: p.editButtonText, CallbackData: p.prefix + cmdEdit},
		)
	}

	if p.removeButtonText != "" {
		controlButtonsRow = append(controlButtonsRow, models.InlineKeyboardButton{
			Text: p.removeButtonText, CallbackData: p.prefix + cmdRemove,
		})
	}

	if len(controlButtonsRow) > 0 {
		kb.InlineKeyboard = append(kb.InlineKeyboard, controlButtonsRow)
	}
